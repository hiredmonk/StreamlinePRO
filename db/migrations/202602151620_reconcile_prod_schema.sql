-- StreamlinePRO production schema reconciliation
-- Date: 2026-02-15
...
begin
  ...
  if not exists (select 1 from pg_type where typname = 'project_privacy') then
...
drop policy if exists "project_sections_select" on project_sections;

create policy "project_sections_select" on project_sections
for select
using (app_user_can_access_project(project_id));

drop policy if exists "project_sections_modify" on project_sections;
...
drop policy if exists "project_statuses_modify" on project_statuses;
...
drop policy if exists "recurrences_modify" on recurrences;
...
drop policy if exists "task_comments_modify" on task_comments;
...
drop policy if exists "task_attachments_modify" on task_attachments;
